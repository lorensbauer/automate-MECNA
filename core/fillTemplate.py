...
def fillTemplate(socio, plantilla_path):
    ...
    replace_word = {
        "{{Izena}}": str(socio["Izena"]),
        "{{Abizenak}}": str(socio["Abizenak"]),
        "{{NAN_zkia}}": str(socio["NAN_zkia"]),
        "{{Zenbatekoa}}": str(socio["Zenbatekoa"]),
...
def funcionConDocxtpl(df):
    ...
    # Renderizar
    doc.render({"socios": socios, "salto": "\n\n\n\n\n\n\n\n", "data": datetime.now().strftime("%Y-%m-%d")})
...
if __name__ == "__main__":
    mecna_names = pd.read_excel(
        "../input/MECNA.xlsx", usecols="A:D", header=0, index_col=None, sheet_name="2025 (uztailetik)"
    )
